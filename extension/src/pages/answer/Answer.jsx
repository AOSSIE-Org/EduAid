...
const Answer = () => {
  ...
  const handleApiCall = async () => {
    ...
    const booleanQuestions = questions
    ...
    try {
      const responses = await Promise.all([
        fetch("http://localhost:5000/get_boolean_answer", {
          ...
          body: JSON.stringify({
            input_text: context,
            input_question: booleanQuestions,
          }),
        }),
        ...
        fetch("http://localhost:5000/get_shortq_answer", {
          ...
          body: JSON.stringify({
            input_text: context,
            input_question: shortQuestions,
          }),
        }),
      ]);

      const [booleanAnswers, mcqAnswers, shortAnswers] = await Promise.all(
        responses.map((res) => res.json())
      );

      const allAnswers = [
        ...(booleanAnswers?.output ?? []).map((answer, index) => ({
          question: booleanQuestions[index],
          answer,
        })),
        ...(mcqAnswers?.output ?? []).map((answer, index) => ({
          question: mcqQuestions[index]?.question,
          answer,
        })),
        ...(shortAnswers?.output ?? []).map((answer, index) => ({
          question: shortQuestions[index],
          answer,
        })),
      ];
    ...
    <div className="popup w-36rem h-38rem bg-[#02000F] flex justify-center items-center">
      ...
      <div className="w-full h-full bg-cust bg-opacity-50 bg-custom-gradient">
        ...
        <div className="mt-2 mx-3">
          <button
            onClick={addQuestion}
            className="bg-[#518E8E] text-sm text-white rounded-xl border border-[#cbd0dc80] px-6 py-1"
          >
            Add Question
          </button>
        </div>
        <div
          className="mt-4 mx-3 h-64 overflow-y-auto"
          style={{ scrollbarWidth: "thin", msOverflowStyle: "auto" }}
        >
          {questions.map((q, index) => (
            <div
              key={index}
              className="mb-2 p-1 border-dotted border-4 border-[#7600F2] rounded-xl"
            >
              <select
                value={q.type}
                ...
                <div className="mt-2">
                  {q.options.map((option, oIndex) => (
                    <div key={oIndex} className="flex mb-1">
                      ...
                      <button
                        ...
                        className="bg-[#3e506380] text-sm text-white rounded-xl border border-[#cbd0dc80] px-2 py-1"
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  {q.options.length < 4 && (
                    <button
                      ...
                      className="bg-[#3e506380] text-sm text-white rounded-xl border border-[#cbd0dc80] px-4 py-1 mt-1"
                    >
                      Add Option
                    </button>
                  )}
              ...
              <button
                ...
                className="bg-[#3e506380] text-sm text-white rounded-xl border border-[#cbd0dc80] px-4 py-1 mt-2 mb-1"
              >
                Remove Question
              </button>
            </div>
        ...
        <div className="flex my-2 justify-center gap-6 items-start">
          <button
            ...
            className="bg-black items-center text-sm text-white rounded-xl px-6 py-2 mx-auto border-gradient"
          >
            Generate Answers
          </button>
        </div>
        <div
          className="mt-4 mx-3 h-16 overflow-y-auto"
          style={{ scrollbarWidth: "thin", msOverflowStyle: "auto" }}
        >
          {answers.map((answer, index) => (
            <div
              key={index}
              className="bg-[#202838] text-white rounded-xl px-5 py-2 mb-2"
            >
