...
class TestDifficultyLevels:
    """Test the difficulty parameter across question generation endpoints."""

    QUESTION_ENDPOINTS: ClassVar[list[str]] = ["/get_mcq", "/get_boolq", "/get_shortq"]

    # -- Difficulty is accepted and echoed back --------------------------------

    @pytest.mark.parametrize("difficulty", ["easy", "medium", "hard"])
    def test_mcq_accepts_all_difficulty_levels(self, client, difficulty):
        """Verify /get_mcq accepts easy, medium, and hard difficulty."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": difficulty},
        )
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    @pytest.mark.parametrize("difficulty", ["easy", "medium", "hard"])
    def test_boolq_accepts_all_difficulty_levels(self, client, difficulty):
        """Verify /get_boolq accepts easy, medium, and hard difficulty."""
        resp = client.post(
            "/get_boolq",
            json={"input_text": SAMPLE_TEXT, "difficulty": difficulty},
        )
        assert resp.status_code == 200
        assert resp.get_json()["difficulty"] == difficulty

    @pytest.mark.parametrize("difficulty", ["easy", "medium", "hard"])
    def test_shortq_accepts_all_difficulty_levels(self, client, difficulty):
        """Verify /get_shortq accepts easy, medium, and hard difficulty."""
        resp = client.post(
            "/get_shortq",
            json={"input_text": SAMPLE_TEXT, "difficulty": difficulty},
        )
    ...
    @pytest.mark.parametrize("difficulty", ["easy", "medium", "hard"])
    def test_problems_accepts_all_difficulty_levels(self, client, difficulty):
        """Verify /get_problems accepts easy, medium, and hard difficulty."""
        resp = client.post(
            "/get_problems",
            json={"input_text": SAMPLE_TEXT, "difficulty": difficulty},
        )
    ...
    def test_default_difficulty_is_medium(self, client, endpoint):
        """Verify omitting difficulty defaults to medium."""
        resp = client.post(endpoint, json={"input_text": SAMPLE_TEXT})
        assert resp.status_code == 200
    ...
    @pytest.mark.parametrize("invalid", ["extreme", "SUPER", "", 123, None, True])
    def test_invalid_difficulty_falls_back_to_medium(self, client, invalid):
        """Verify invalid difficulty values fall back to medium."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": invalid},
        )
        assert resp.status_code == 200
        assert resp.get_json()["difficulty"] == "medium"
    ...
    @pytest.mark.parametrize("variant", ["EASY", "Easy", "eAsY", "HARD", "Hard"])
    def test_difficulty_is_case_insensitive(self, client, variant):
        """Verify difficulty parameter is case-insensitive."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": variant},
        )
        assert resp.status_code == 200
        assert resp.get_json()["difficulty"] == variant.lower()

    # -- Easy MCQ: fewer distractor options ------------------------------------

    def test_easy_mcq_has_at_most_2_options(self, client):
        """Verify easy difficulty trims MCQ options to at most 2."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": "easy"},
        )
        assert resp.status_code == 200
        questions = resp.get_json()["output"]
        for q in questions:
            assert len(q.get("options", [])) <= 2
            assert "extra_options" not in q

    # -- Medium MCQ: default 3 options (unchanged) -----------------------------

    def test_medium_mcq_has_default_options(self, client):
        """Verify medium difficulty keeps the default 3 MCQ options."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": "medium"},
        )
        assert resp.status_code == 200
        questions = resp.get_json()["output"]
        for q in questions:
            assert len(q.get("options", [])) == 3

    # -- Hard MCQ: expanded options (up to 5) ----------------------------------

    def test_hard_mcq_has_expanded_options(self, client):
        """Verify hard difficulty expands MCQ options using extra_options."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": "hard"},
        )
        assert resp.status_code == 200
        questions = resp.get_json()["output"]
        for q in questions:
            # Should have more than the default 3 (merged from extra_options)
            assert len(q.get("options", [])) > 3
            assert "extra_options" not in q

    # -- Hard questions have difficulty tag -------------------------------------

    def test_mcq_questions_have_difficulty_tag(self, client):
        """Verify each MCQ question includes a difficulty field."""
        resp = client.post(
            "/get_mcq",
            json={"input_text": SAMPLE_TEXT, "difficulty": "hard"},
        )
    ...
    def test_shortq_questions_have_difficulty_tag(self, client):
        """Verify each short question includes a difficulty field."""
        resp = client.post(
            "/get_shortq",
            json={"input_text": SAMPLE_TEXT, "difficulty": "easy"},
        )
