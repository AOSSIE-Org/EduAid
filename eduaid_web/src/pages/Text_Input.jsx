  ...
  const getEndpoint = (difficulty, questionType) => {
    ...
    return questionType;
      ...
      const handleAnswer = (questionIndex, optionIndex) => {
        ...
        updatedQuestions[questionIndex].selected = selectedOption;
        
        setQuestions(prev =>
          prev.map((q,i) =>
          i === questionIndex ? { ...q, selected: selectedOption } : q
          )
        );
        apiClient.post("/submit_answer", {
          questionIndex,
          selectedOption,
        })
         .then(res => console.log("Backend adaptive response:", res))
      ...
      setQuestions(responseData.output);
      if (responseData.next_difficulty){
        setDifficulty(responseData.next_difficulty.charAt(0).toUppercase() + responseData.next_difficulty.slice(1)+ "Difficulty");
      }
          ...
          {questions.length > 0 && (
            <div className="mt-8">
              {questions.map((q, index) => (
                <div key={index} className="bg-[#1c1c1c] p-4 rounded-xl my-4">
                  ...
                  {q.options && q.options.map((opt, i) => (
                    <button
                      ...
                      className="block mt-2 px-4 py-2 bg-[#3e5063] text-white rounded-lg"
                    >
                      {opt}
                      </button>
                    ))}
